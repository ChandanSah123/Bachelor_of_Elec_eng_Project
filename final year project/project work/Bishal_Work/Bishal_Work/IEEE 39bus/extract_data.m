...
function [line_data, trans_data, load_data, generator_data1, generator_data2, mechanical_data] = extract_data()
try
    modelName = bdroot;
...
for i=1:length(transBlocks)
    ...
    trans_data(i).PrimaryRatedVoltageVp = eval(get_param(block_path1, 'VRated1'));     % KVA
    trans_data(i).SecondaryRatedVoltageVs = eval(get_param(block_path1, 'VRated2'));    % KVA
end
